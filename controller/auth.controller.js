...
export const signUp = async (req, res, next) => {
    ...
    try {
        ...
        const newUser = await Users.create([{ name, email, password: hashedPassword }], { session });

        const token = jwt.sign({ userId: newUser[0]._id, email }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
